 ...
 */
/* ----------------------------------------------------------------------------*/
typedef void (TOV::*LSODA_ODE_SYSTEM_TYPE)(double t, double *y, double *dydt);

// using LSODA_ODE_SYSTEM_TYPE = void (TOV::*)(double t, double *y, double *dydt);
...
public:
    ...
    void prja(const size_t neq, vector<double> &y, TOV* model, LSODA_ODE_SYSTEM_TYPE f);

    void lsoda(TOV* model, LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y, double *t,
        double tout, int itask, int *istate, int iopt, int jt, array<int, 7> &iworks,
        array<double, 4> &rworks);

    void correction(const size_t neq, vector<double> &y, TOV* model, LSODA_ODE_SYSTEM_TYPE f,
        size_t *corflag, double pnorm, double *del, double *delp, double *told,
        size_t *ncf, double *rh, size_t *m);

    void stoda(const size_t neq, vector<double> &y, TOV* model, LSODA_ODE_SYSTEM_TYPE f);

    // We call this function in VoxelPools::
    void lsoda_update(TOV* model, LSODA_ODE_SYSTEM_TYPE f, const size_t neq, vector<double> &y,
        std::vector<double> &yout, double *t, const double tout, int *istate, double rtol = 1e-6, double atol = 1e-6  // Tolerance
    );
