...
class MassProfile extends PureComponent {
  CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip">
          <p className="tooltip-label">{`m : ${payload[0].value.toFixed(
            4
          )}`}</p>
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Line type="monotone" dataKey="mass" fill="#39a2db" />
      </LineChart>
    );
  }
}

export default React.memo(MassProfile);
