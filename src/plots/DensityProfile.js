...
class DensityProfile extends PureComponent {
  DataFormater = (number) => {
    number = number !== 0 ? number.toFixed(3) : number;
    return number !== 0 ? number.toString() + "e-2" : number.toString();
  };
  CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length) {
      return (
        <div className="custom-tooltip">
          <p className="tooltip-label">{`ρ : ${payload[0].value.toFixed(
            4
          )}e-2`}</p>
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        />
        <Line type="monotone" fill="#39a2db" dataKey="density" />
      </LineChart>
    );
  }
}

export default React.memo(DensityProfile);
